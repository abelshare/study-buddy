...
| 场景 | 现实 | Python |
|------|------|--------|
| **全局环境** | 工具箱放公司仓库，所有人共用 | 系统 Python + 全局包 |
| **问题** | 项目A要锤子v1，项目B要锤子v2，冲突 | 版本冲突 |
| **虚拟环境** | 每个项目带自己的工具箱 | 每个项目独立的依赖空间 |
...
| 自动使用依赖 | ✅ 进入目录自动识别 | ❌ 需手动激活环境 |
| 提交到 Git | ❌ 不提交 node_modules | ❌ 不提交 venv |

**Python 需要显式激活**：
```bash
# Node.js：自动使用项目依赖
cd my-project
node app.js  # 自动找到 node_modules

# Python：必须先激活
cd my-project
.\venv\Scripts\activate  # 先激活
python app.py            # 才使用 venv 里的包
```

### 项目结构对照

```
Node.js 项目                      Python 项目
├── index.js                      ├── main.py          # 你的代码
...
flowchart TB
    ...
    B --> C[激活环境<br/>.\venv\Scripts\activate]
    C --> D[安装依赖<br/>pip install -r requirements.txt]
    D --> E[运行项目<br/>python main.py]
    
    F[添加新依赖] --> G[pip install 包名]
    G --> H[导出依赖<br/>pip freeze > requirements.txt]
    H --> I[提交 requirements.txt]
```
...
由 Astral 公司用 Rust 开发，整合多个工具于一身，速度比 pip 快 10-100x。
...
# 运行代码
uv run python main.py     # 自动管理环境并运行
...
uv tool run ruff check .  # 一次性运行工具
```
...
# uv 方式（1 步）
uv run python main.py     # 自动创建环境、安装依赖、运行
```
...
> **注意**：uv 已整合 pyenv 功能，使用 `uv python install` 即可管理版本。
...
|------|---------|------|
| 新项目 / 追求效率 | **uv** ⭐ | 快、简单、全能 |
| 数据科学 / ML | uv 或 Pixi | 依赖管理友好 |
| 团队协作 / 企业 | Poetry 或 uv | 成熟稳定 |
| 简单脚本 / 学习 | pip + venv | 无需额外安装 |
| CI/CD 环境 | uv ⭐ | 速度优势明显 |
